...
describe("Operaciones iterables", () => {
    ...
    it("(Bonus) Razas de gatos que contienen la letra O", () => {

        let gatos = ["Exótico", "Persa", "Siamés", "Maine Coon", "British", "Ragdoll"];

        let RazasConO = gatos.filter(raza => raza.includes('o'));
         
        expect(RazasConO).to.have.same.members(["Exótico", "Maine Coon", "Ragdoll"]);
    })
})